...
```{r}
fars_datafile_path <- "/tmp/data/fars-datafile.csv.bz2" 
...
filename_fars_data <- make_filename(year = year_fars_data)
```
...
fars_data_by_year <- fars_read_years(years = years_fars_data)
```
...
```{r eval=FALSE}
years_fars_data <- c(2013, 2014) # or c("2013", "2014")
summary_fars_accidents_by_year <- fars_summarize_years(years = years_fars_data)
```
